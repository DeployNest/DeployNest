...
class User {
	...
	private async authWithPassword(password: string): Promise<boolean> {
		...
		const userData = await databases
			.getUserCollection()
			.then((users) => {
	...
	private async authWithOneTimeCode(oneTimeCode: string): Promise<boolean> {
		...
		const validCode = userData.oneTimeCodes.find(
			(code) => code.code === oneTimeCode && code.expiresAt > new Date()
		);
	...
	async generateDeviceToken(): Promise<string> {
		...
		const token = jwt.sign(
			{ username: this.username, type: "device", jti: tokenId },
			secret,
			{ expiresIn: "30d" }
		);
	...
	async generateBearerToken(): Promise<string> {
		...
		const token = jwt.sign(
			{ username: this.username, type: "bearer", jti: tokenId },
			secret,
			{ expiresIn: "7d" }
		);
